...
const CartPage = () => {
  ...
  return <div className="pt-24 pb-16">
      <div className="max-w-6xl mx-auto px-4">
        ...
        {!showCheckout ? <div className="grid lg:grid-cols-3 gap-8">
            {/* Cart Items */}
            <div className="lg:col-span-2 space-y-4">
              {items.map((item, index) => <div key={item.id} className="fade-up flex items-center space-x-4 bg-white p-4 rounded-lg border border-clay/30" style={{
            animationDelay: `${index * 0.1}s`
          }}>
                  <Link to={`/product/${item.id}`} className="flex-shrink-0">
                    <img src={item.image} alt={item.name} className="w-20 h-20 object-cover rounded-lg hover:opacity-80 transition-opacity cursor-pointer" />
                  </Link>
                  ...
                  <div className="flex items-center space-x-2">
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1 hover:bg-clay/30 rounded">
                  ...
                  <button onClick={() => removeFromCart(item.id)} className="p-2 hover:bg-red-100 text-red-600 rounded">
          ...
          animationDelay: '0.3s'
              ...
              <div className="bg-blush/30 p-6 rounded-lg sticky top-32">
                ...
                <div className="space-y-3">
                  <Button onClick={() => setShowCheckout(true)} className="w-full bg-charcoal hover:bg-ash text-linen">
                    Proceed to Checkout
                  </Button>
                  <Link to="/products" className="block">
                    <Button variant="outline" className="w-full border-clay bg-yellow-600 hover:bg-yellow-500 text-slate-50">
                      Continue Shopping
                    </Button>
                  </Link>
                </div>
